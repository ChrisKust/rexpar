predict_lin1_extra <- function(y, CritLen, CritTime, NSim, alpha, restrict = FALSE, 
                               start, dmax_neu, res_rescale, lambda_j, eps = 1e-9)
{
  ...
  for(i in 1:NSim)
  {
    ...
    jumpoc <- rbinom(predLength, size = 1, prob = lambda_j)
    Res_t <- jumpoc * sample(Res_max, predLength, replace = TRUE) * res_rescale + (1 - jumpoc) * sample(Res_norm, predLength, replace = TRUE)
