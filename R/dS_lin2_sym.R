dS_lin2_sym <- function(theta, resy, y, ncores = 1, model = c("linAR1", "nlinAR1", "linARc"), cpow = 1) {
  ...
  if ((is.numeric(ncores) == TRUE) && (ncores > 1) && (ncores <= parallel::detectCores() * 2)) {
    ...
    Matsd <- sum(unlist(parLapply(cl, Reslist, oner_sym)))
    stopCluster(cl)
  }
  
  if (ncores == 1) {
    Matsd <- sum(unlist(lapply(Reslist, oner_sym)))
  }
  ...
  Matsd <- Matsd + 1/3*((resy[length(resy) - 2] > 0) * (resy[length(resy) - 1] < 0) *
    (resy[length(resy)] > 0) + (resy[length(resy) - 2] < 0) * 
    (resy[length(resy) - 1] > 0) * (resy[length(resy)] < 0)
  + (resy[length(resy) - 2] > 0) * (resy[length(resy) - 1] > 0) *
    (resy[length(resy)] < 0)
  + (resy[length(resy) - 2] < 0) * (resy[length(resy) - 1] < 0) *
    (resy[length(resy)] > 0)
  + (resy[length(resy) - 2] < 0) * (resy[length(resy) - 1] > 0) *
