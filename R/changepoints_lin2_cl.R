changepoints_lin2_cl <- function(y, level, bw, sw, plots = FALSE, method = "m1", cluster, mincper = 1, mincp = 1)
{
alpha <- (1 - level)
...
{
  ASp1 <- lin2_CI_cl(y[(cands[i] - bw):cands[i]], (1 - alpha), notion = "dS_pre", cluster = cluster)
  ASp2 <- lin2_CI_cl(y[cands[i]:(cands[i] + bw)], (1 - alpha), notion = "dS_pre", cluster = cluster)
  ...
  changeind[i] <- try(1 - (convex_hull_intersect(cbind(ASp1$par[!ASp1$inCI, 1], ASp1$par[!ASp1$inCI, 2]), cbind(ASp2$par[!ASp2$inCI, 1], ASp2$par[!ASp2$inCI, 2]), y1 = y[(cands[i] - bw):cands[i]],y2 = y[cands[i]:(cands[i] + bw)])$sumint > 0), silent=T)
}
  unlist(changeind)
},
